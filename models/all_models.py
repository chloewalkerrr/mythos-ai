...
"""
from sqlalchemy import Column, Integer, String, Text, Date, Float, Boolean, ForeignKey, CheckConstraint, Index, Enum
...
class Monster(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("threat_level IN ('low', 'medium', 'high', 'extreme')", name='check_threat_level'),
    )
...
class Location(Base):
    ...
    quests_ending_here = relationship("Quest", foreign_keys="[Quest.end_location_id]", back_populates="end_location")
...
class Quest(Base):
    ...
    difficulty_level = Column(Integer, CheckConstraint('difficulty_level >= 1 AND difficulty_level <= 10'))
    ...
    end_location = relationship("Location", foreign_keys=[end_location_id], back_populates="quests_ending_here")
    ...
    monsters_encountered = relationship("QuestMonster", back_populates="quest", cascade="all, delete-orphan")
    
    __table_args__ = (
        Index('idx_quest_status', 'status'),
        CheckConstraint("status IN ('pending', 'in_progress', 'completed', 'failed')", name='check_quest_status'),
    )
...
class CharacterPower(Base):
    ...
    __table_args__ = (
        Index('idx_character_power', 'character_id', 'power_id'),
    )
...
class QuestParticipant(Base):
    ...
    __table_args__ = (
        Index('idx_quest_participant', 'quest_id', 'character_id'),
    )
    
    def __repr__(self):
        return f"<QuestParticipant(quest_id={self.quest_id}, character_id={self.character_id}, role='{self.role}')>"
